...
#we count word frequencies keeping a column for the user where the word comes from
yt_words
```
...
# Create a column called author for each dataframe
freq <-bind_rows(
  mutate(tidy_yt, platform = "YouTube"),
  mutate(tidy_reddit, platform = "Reddit")
) |>  
  # Regex to identify words and not _words_
  mutate(word = str_extract(word, "[a-z']+")) |> 
  # Count the number of mentions of a word for an author
  count(platform, word) |> 
  # Calculate proportion over the total sum of words
  group_by(platform) |> 
  mutate(proportion = n / sum(n)) |>  
  select(-n) |> 
  # Reshape the dataframe
  # Pivot wider means: more columns, less rows
  pivot_wider(names_from = platform, values_from = proportion) 
...
```{r}
library(ggplot2)
library(scales)  # Make sure you have scales loaded for percent_format()

ggplot(freq, aes(x = Reddit, y = YouTube, fill = word)) +
...
freq %>%
  ...
  comparison.cloud(colors = c("blue", "green"), max.words = 50, scale = c(3, 0.5))
```
### WordCloud2
```{r}
freq |> 
  inner_join(get_sentiments("bing")) |> 
  count(word, c(sentiment, Reddit, YouTube), sort = TRUE) |> 
